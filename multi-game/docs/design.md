...
| 4 | 스피드 퀴즈 추가 (동시참여·속도 보너스 점수) | ✅ |
| + | 오목 추가 (2인 턴제, 5목 승리 판정) | ✅ |
| + | 망치 연타 / 연타 달리기 추가 (연타형, 캔버스 애니메이션) | ✅ |
| 5 | 관전 모드 + 결과화면 + 로비 폴리시 | ✅ |
| + | 팀전(개인전/팀전 토글) — 단체 게임 한정 | ✅ |

### 팀전 (개인전/팀전)

- **단체 게임(supportsTeams)** 에서만 방장이 개인전/팀전 토글. 오목(1:1)은 제외.
- 배정: **자기 팀 선택(🔴 레드/🔵 블루) + 자동 밸런스**. 시작 조건에 "각 팀 1명 이상".
- **팀 점수는 코어가 개인 점수를 팀별 합산**해 생성 → 단체 게임 5종은 게임 로직 수정 없이 팀전 지원. GAME OVER를 팀 순위(승리팀) + 개인 기록으로 표시.
- `Player.team`을 게임에 노출(팀 색 표시 등 확장 여지), 재접속 시에도 팀 유지.

### 연타형 게임 (애니메이션 연출)

- **망치 연타(hammer-smash)**: 제한시간 동안 연타 → 횟수로 순위. 클라가 requestAnimationFrame 캔버스로 파티클(스파크/"+1")·화면 흔들림·카운트 팝, CSS로 망치 스윙·두더지 찌부러짐. 서버는 카운트·라운드 타이머·순위, **연타 레이트 리밋(45ms)** 으로 매크로 방지. `onTick`으로 리더보드 스로틀 전송.
- **연타 달리기(mash-race)**: 연타로 전진, 먼저 결승선 도착 순으로 순위. 레인별 러너를 서버 거리로 갱신 + CSS transition 보간, 연타 시 홉·먼지 연출.
- 공통 설계: 연출은 클라(각 게임 mount 내부), 카운트·판정·순위는 서버. **키 auto-repeat 무시**(홀드로 연타 불가) + 서버·클라 동일 레이트 리밋으로 낙관적 표시가 서버와 일치.

### Phase 5 — 관전 모드 & 로비 폴리시

- **관전(spectator)**: 룸에 `spectators`(플레이어와 별개) 개념 추가. `room:spectate`로 진행중/만석 방을 관전. 관전자는 `game:state`를 동일하게 받되(게임 입장에서 players에 없는 id → 외부 시점 뷰), **서버가 관전자의 `game:action`을 차단**하고 클라이언트는 게임 마운트에 `pointer-events:none` + SPECTATING 오버레이.
- `room:role` 이벤트로 클라이언트가 자신의 역할(플레이어/관전자)을 인지. 재접속 시에도 역할·상태 복원.
- **로비 폴리시**: 방 정렬(대기중 우선), `게임중` 라이브 배지 + 👁 관전 버튼, 관전자 수 표시.
- 플레이어가 모두 나가면 방 종료(관전자는 로비로).

## UI 테마 (아케이드)

- 다크 배경 + 신스웨이브 그리드 + CRT 스캔라인, 픽셀 폰트(Galmuri11 / Press Start 2P).
- 네온 글로우 3D 아케이드 버튼, 모니터 베젤형 캔버스, 하이스코어 점수판, 터미널 피드.
- 타이틀 네온 호흡/깜빡임 애니메이션, 화면·게임 진입 시 CRT 전원 ON 효과, 결과는 "GAME OVER / HI-SCORE" 연출.
